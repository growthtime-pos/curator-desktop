...
## Handoff
- Goal: 사용자 지정 Atlassian 환경 변수(ATLASSIAN_URL/ATLASSIAN_ID/ATALSSIAN_PW)를 Codex/GitHub 양쪽에서 읽어 검증
- Changed:
  - config loader에 ATLASSIAN alias 우선순위 추가(typo key `ATALSSIAN_PW` 포함)
  - `test_config_env.py`로 alias/fallback 로딩 테스트 추가
  - GitHub Actions에서 vars/secrets로 ATLASSIAN_* 읽고 필수값 검증 후 테스트 실행하도록 단계 추가
- Contracts Updated:
  - API 계약 변경 없음
- Validation:
  - `python -m compileall apps/backend/src`
  - `cd apps/backend && python -m unittest discover -s tests -p 'test_*.py'`
  - `python - <<'PY' ... load_config() ... PY` (Codex env read check)
- Next Actions:
  - 필요 시 GitHub Action의 필수값 실패 정책을 optional 모드로 완화 검토
- Blockers:
  - 없음
